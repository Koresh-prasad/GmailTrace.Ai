...
start "MailShield Python Backend (Port 5000)" cmd /k "cd /d %~dp0backend-python && .venv\Scripts\python.exe -m uvicorn main:app --port 5000 --host 0.0.0.0 --reload"

echo [*] Waiting for Backend to initialize...
timeout /t 3 /nobreak >nul

echo [*] 2/2 Starting React Frontend on Port 5173...
start "MailShield React Frontend (Port 5173)" cmd /k "cd /d %~dp0frontend && npm.cmd run dev"
